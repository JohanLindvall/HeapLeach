...
| **KVS searches** | `/search/<query>/` on any KVS install | Everything the search turns up, walked through the same block loader. Past its end that loader is a 404 rather than page one again, but the last page says it is the last in its own pager, so the walk stops there without asking. Results also list the albums matching the query, in a block with a pager of its own, which is why both are picked by name rather than by coming first. |
...
179 sites across 92 extractors, plus 6 that match by shape rather than by host.
