...
## hosts: regenerate the supported-site list in README.md from the registry
hosts:
ifndef HAVE_GO
	$(error Go is not installed — the host list is generated by the program itself)
endif
	@cd backend && go run ./cmd/hostlist -write ../README.md

## hosts-check: fail if README.md's host list is not what the code produces
#
# Regenerated into a copy and compared against the file itself, rather than
# written in place and compared against git: the section is then checked in
# whatever state the tree is in, so a README with prose edits in progress
# does not read as a stale inventory, and CI — where the tree is the commit
# — still fails on a regeneration that was never committed.
hosts-check:
ifndef HAVE_GO
	$(error Go is not installed — the host list is generated by the program itself)
endif
	@tmp=$$(mktemp) && cp README.md "$$tmp" && \
	  (cd backend && go run ./cmd/hostlist -write "$$tmp" >/dev/null) && \
	  if ! cmp -s README.md "$$tmp"; then \
	    echo "README.md's host list is out of date — run 'make hosts' and commit"; \
	    diff -u README.md "$$tmp" | head -40; rm -f "$$tmp"; exit 1; \
	  fi; rm -f "$$tmp"
	@echo ">> host list is current"

## test: run the Go unit tests, with the race detector as CI does
test:
...
fmt-check:
	@unformatted=$$(cd backend && gofmt -l ./internal ./cmd); \
...
## vet: run go vet, over the live tests too
vet:
	cd backend && go vet ./... && go vet -tags live ./...
...
distclean: clean
	...
	rm -f $(BIN_DIR)/yt-dlp $(BIN_DIR)/ffmpeg $(BIN_DIR)/ffprobe $(BIN_DIR)/deno
